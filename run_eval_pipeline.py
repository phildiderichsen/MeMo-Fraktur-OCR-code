...
def main():
    ...
    safe_copytree(conf['orig_page_dir'], pth.eval.ocr_kb_dir)
    safe_copytree(conf['gold_page_dir'], pth.eval.gold_novels_dir)
    ...
    tess_outdirs = [os.path.join(pth.eval.intermediate, f'tess_out_{label}') for label in traineddata_labels]
    uncorrected_dir = os.path.join(pth.eval.intermediate, conf['base_ocr'])
    corrected_dir = os.path.join(pth.eval.intermediate, pth.param_str)
    ...
    if conf.getboolean('run_pdf2img'):
        pdfs2imgs(pth.eval.pdf_paths, pth.eval.img_dir, int(conf['split_size']))
    if conf.getboolean('run_ocr'):
        do_ocr(pth.eval.img_dir, pth.eval.intermediate, traineddata_labels)
    ...
    if conf.getboolean('correct_hard'):
        correct_hard_fraktur_errors(uncorrected_dir, pth.eval.intermediate, corrected_dir)
        uncorrected_dir = corrected_dir
    ...
    if conf.getboolean('make_basic_gold_vrt'):
        gold_vrt_gen = generate_novels_vrt(pth.eval.gold_novels_dir, pth.eval.corp_label)
        write_novels_vrt(gold_vrt_gen, pth.eval.basic_gold_vrt_path)
    if conf.getboolean('annotate_gold_vrt'):
        text_annotation_generator = generate_gold_annotations(pth.eval.basic_gold_vrt_path,
                                                              pth.eval.ocr_kb_dir,
                                                              conf['texton_out_dir'],
                                                              pth.eval.corp_label,
                                                              tess_outdirs,
                                                              ...
                                                              conf)
        write_annotated_gold_vrt(text_annotation_generator, pth.eval.local_annotated_gold_vrt_path)
        shutil.copy(pth.eval.local_annotated_gold_vrt_path, pth.eval.annotated_gold_vrt_path)
    if conf.getboolean('analyze_errors'):
        # TODO Not very transparent error when n_datasets is wrong.
        analyze_gold_vrt(pth.eval.annotated_gold_vrt_path, conf, pth.eval.analyses_dir, pth.param_str, n_datasets=5)
    if conf.getboolean('write_korp_configs'):
        ...
        shutil.copy(conf['frakturgold_mode_outpath'], os.path.join(pth.eval.vrt_dir, 'memo_frakturgold_mode.js'))
        util.write_frakturgold_encodescript(conf['frakturgold_encode_template'],
                                            pth.eval.annotated_outdir,
                                            conf['gold_vrt_p_attrs'],
                                            conf['frakturgold_encode_outpath'])
        shutil.copy(conf['frakturgold_encode_outpath'], os.path.join(pth.eval.vrt_dir, 'encode_MEMO_fraktur_gold.sh'))
