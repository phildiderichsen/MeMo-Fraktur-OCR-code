...
def main():
    ...
    tess_outdirs = [os.path.join(pth.corr.fulloutputdir, f'tess_out_{label}') for label in traineddata_labels]
    uncorrected_dir = os.path.join(pth.corr.fulloutputdir, conf['base_ocr'])
    corrected_dir = os.path.join(pth.corr.fulloutputdir, pth.param_str)
    ...
    if conf.getboolean('run_pdf2img'):
        pdfs2imgs(pth.corr.frakturpaths, pth.corr.img_dir, int(conf['split_size']))
    if conf.getboolean('run_ocr'):
        do_ocr(pth.corr.img_dir, pth.corr.fulloutputdir, traineddata_labels)
    ...
    if conf.getboolean('correct_hard'):
        correct_hard_fraktur_errors(uncorrected_dir, pth.corr.fulloutputdir, corrected_dir)
        uncorrected_dir = corrected_dir
    ...
    #     gold_vrt_gen = generate_novels_vrt(pth.corr.gold_novels_dir, pth.corr.corp_label)
    #     write_novels_vrt(gold_vrt_gen, pth.corr.basic_gold_vrt_path)
    # if conf.getboolean('annotate_gold_vrt'):
    #     text_annotation_generator = generate_gold_annotations(pth.corr.basic_gold_vrt_path, pth.corr.ocr_kb_dir,
    #                                                           conf['texton_out_dir'], pth.corr.corp_label, tess_outdirs,
    #                                                           [corrected_dir], conf)  # TODO single dir instead of list of dirs?
    #     write_annotated_gold_vrt(text_annotation_generator, pth.corr.local_annotated_gold_vrt_path)
    #     shutil.copy(pth.corr.local_annotated_gold_vrt_path, pth.corr.annotated_gold_vrt_path)
    # if conf.getboolean('analyze_errors'):
    #     # TODO Not very transparent error when n_datasets is wrong.
    #     analyze_gold_vrt(pth.corr.annotated_gold_vrt_path, conf, pth.corr.analyses_dir, param_str, n_datasets=5)
    ...
    #     shutil.copy(conf['frakturgold_mode_outpath'], os.path.join(pth.corr.vrt_dir, 'memo_frakturgold_mode.js'))
    #     util.write_frakturgold_encodescript(conf['frakturgold_encode_template'],
    #                                         pth.corr.annotated_outdir,
    #                                         conf['gold_vrt_p_attrs'],
    #                                         conf['frakturgold_encode_outpath'])
    #     shutil.copy(conf['frakturgold_encode_outpath'], os.path.join(pth.corr.vrt_dir, 'encode_MEMO_fraktur_gold.sh'))
